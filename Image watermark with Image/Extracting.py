...
def extract_watermark_grayscale(image, alpha=0.05, watermark_size=None):
    """
    ...
    - watermark_size: Size of the watermark used during embedding (height, width).
...
try:
    ...
    alpha_values = [round(x * 0.01, 2) for x in range(1, 31)]  # alpha from 0.01 to 0.3

    # Loop through alpha values to extract and save the watermark images
    for alpha in alpha_values:
        extracted_watermark = extract_watermark_grayscale(
            'C:\\Users\\sakshita\\OneDrive\\Desktop\\Mini Project\\Code\\water12.jpg',  # Replace with the path to the watermarked image
            alpha=alpha,
            watermark_size=(3776,6720)  # Replace with the original watermark size (height, width)
        )
        
        # Save the extracted watermark for this alpha value
        output_filename = os.path.join(output_folder, f'extracted_watermark_alpha_{alpha:.2f}.jpg')
